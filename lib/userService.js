...
export async function adminUpdateUser(id, data) {
	...
	const { name, email, password, slackId, clickupId, isAdmin, isExternal, isArchived } = data || {};
 ...
 * @returns {Promise<{ user: object, plainPassword: string }>} safe user doc and plaintext password (for welcome email only)
 */
export async function createExternalUser(data) {
	...
	const safe = user.toObject();
	delete safe.password;
	return { user: safe, plainPassword };
}