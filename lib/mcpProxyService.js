...
import { assertMcpProxyCustomerData } from "@root/lib/mcpCustomerIdGuard";
import {
...
import { McpRouteNotAllowlistedError } from "@root/lib/mcpRouteAccessErrors";
import {
    buildBlockedRouteAccessReason,
    buildMcpRequestedBy,
} from "@root/lib/mcpRouteAccessHelpers";
import { isMcpApexRouteAccessGranted, logBlockedRouteAccessRequest } from "@root/lib/mcpRouteAccessService";

/**
 * @param {string} route
 * @param {string} customerId
 * @param {Record<string, unknown>} [params]
 * @param {{ keyId?: string, authMethod?: string, email?: string | null, accessReason?: string }} [auditContext]
 */
export async function executeMcpApexProxy(route, customerId, params = {}, auditContext = {}) {
    ...
    const allowed = await isMcpApexRouteAccessGranted(normalizedRoute, id);
    if (!allowed) {
        const query = normalizeProxyParams(params);
        let requestResult = null;
        try {
            requestResult = await logBlockedRouteAccessRequest({
                route: normalizedRoute || route,
                customerId: id,
                reason: buildBlockedRouteAccessReason(
                    normalizedRoute || route,
                    query,
                    auditContext.accessReason
                ),
                requestedBy: buildMcpRequestedBy(auditContext),
            });
        } catch (e) {
            console.error("[mcp route access auto-log]", e);
        }
        throw new McpRouteNotAllowlistedError(
            normalizedRoute || route,
            id,
            requestResult
        );
    ...
    assertMcpProxyCustomerData(id, data, `apex:${normalizedRoute}`);
...
export async function executeMcpShopifyProxyCall(
    ...
    assertMcpProxyCustomerData(id, data, `shopify:${type}`);
...
export async function executeMcpGoogleAdsProxyCall(
    ...
    assertMcpProxyCustomerData(id, data, "google-ads");
...
export async function executeMcpMetaProxyCall(
    ...
    assertMcpProxyCustomerData(id, data, `meta:${ep}`);
...
export function listMcpProxyCatalog() {
    return {
        ...
        guardrails: {
            ...
            sessionRequestSerialization:
                "mcp-server-apex serializes /mcp POST requests per session to prevent crossed tool responses under parallel calls",
        },
        apex: listMcpApexProxyRoutes(),
        apexApprovable: listMcpApexApprovableRoutes(),
        routeAccessRequests: {
            tool: "request_route_access",
            adminReviewUrl: "https://apex.searchmind.tech/admin/route-requests",
            note: "Approvable routes require admin approval per customer before call_apex_api succeeds.",
        },
        shopify: {
