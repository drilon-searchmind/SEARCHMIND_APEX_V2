...
import { fetchMcpMetaCatalogProducts } from "@root/lib/mcpMetaCatalog";
...
export const MCP_APEX_PROXY_ROUTES = {
    ...
    "/api/meta-ad-creatives": {
        description:
            "Live Meta/Facebook ads with creative thumbnails (default: ACTIVE ads only). Optional limit, activeOnly, includeCatalogProducts.",
        method: "GET",
        requiredParams: [],
        optionalParams: ["limit", "activeOnly", "includeCatalogProducts", "catalogMaxProducts"],
        async execute(customerId, params) {
            return fetchMcpMetaAdCreatives(customerId, params);
        },
    },
    "/api/meta-catalog-products": {
        description:
            "Meta Commerce catalog product images for DPA (retailer_id, image_url, name). Cached ~6h. Optional catalogId, productIds, retailerIds.",
        method: "GET",
        requiredParams: [],
        optionalParams: ["catalogId", "productIds", "retailerIds", "maxProducts"],
        async execute(customerId, params) {
            return fetchMcpMetaCatalogProducts(customerId, params);
        },
    },
};
