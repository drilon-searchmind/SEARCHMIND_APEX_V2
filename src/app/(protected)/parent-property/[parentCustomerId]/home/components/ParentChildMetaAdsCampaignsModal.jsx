    ...
    return (
        ...
        >
            ...
            >
                ...
                <h2 id="meta-ads-campaigns-modal-title" className="apex-perf-modal__title">
                    Meta Ads campaigns
                </h2>
                <p className="apex-perf-modal__lede">{propertyLabel}</p>
                <p className="apex-perf-modal__lede mb-0">
                    Check campaigns to exclude from Meta ad spend for this property only.
                </p>
                ...
                <div className="apex-perf-modal__body">
                    <div className="mb-4">
                        ...
                        <div className="apex-parent-modal-toolbar">
                            <div className="flex gap-2">
                                <button type="button" className="apex-parent-modal-link" disabled={controlsDisabled || loading || filteredIds.length === 0} onClick={handleSelectAll}>Select all</button>
                                <span className="text-[var(--color-muted)]">|</span>
                                <button type="button" className="apex-parent-modal-link" disabled={controlsDisabled || loading || filteredIds.length === 0} onClick={handleDeselectAll}>Deselect all</button>
                            </div>
                    ...
                    <div className="min-h-[200px]">
                        {loading ? (
                            ...
                            <p className="apex-parent-empty-note text-center py-8">
                                {campaigns.length === 0 ? "No campaigns found for this date range." : "No campaigns match your search."}
                            </p>
                        ) : (
                            <ul className="apex-parent-modal-list">
                                {filtered.map((c) => {
                                    ...
                                    return (
                                        <li key={id}>
                                            <label>
                                                <input type="checkbox" className="apex-parent-checkbox mt-0.5" checked={excluded} disabled={controlsDisabled} onChange={() => handleToggle(id, !excluded)} />
                                                <span className="flex-1 min-w-0">
                                                    <span className="apex-parent-modal-list__name">{c.name || id}</span>
                                                    <span className="apex-parent-modal-list__meta">ID {id}</span>
                                                </span>
                                            </label>
                                        </li>
                                    );
                    ...
                    <CampaignExclusionKeywordEditor keywords={localKeywords} onChange={setLocalKeywords} disabled={controlsDisabled || loading} />
                </div>

                <div className="apex-perf-modal__footer apex-perf-modal__actions">
                    <button type="button" onClick={onClose} disabled={controlsDisabled} className="apex-perf-btn">Cancel</button>
                    <button type="button" className="apex-perf-btn apex-perf-btn--primary" disabled={controlsDisabled || loading} onClick={handleApplyClick}>
