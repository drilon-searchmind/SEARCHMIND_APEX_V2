...
export default function MarketsMetricsDataRow({
            ...
            case 'orders':
                return (
                    <td
                        key={key}
                        className={cellClass}
                        style={getCellStyles(row.orders, max.orders, row.orders === max.orders)}
                    >
                        {row.orders}
                    </td>
                );
    ...
    const rowClass = isCobalt
        ? `${marketsRowClass(variant, 'data', index)}${!included ? ' is-excluded' : ''}${isSpecial ? ' is-special' : ''}`
        : `${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} ${!included ? 'opacity-40' : ''} ${isSpecial ? 'font-semibold' : ''}`;

    return (
        <tr className={rowClass}>
            <td className={isCobalt ? 'apex-markets-table__cell-include' : 'px-2 py-2 w-10 text-center align-middle'}>
                {!isSpecial && onToggleIncluded ? (
                    <input
                        type="checkbox"
                        className={isCobalt ? 'apex-markets-checkbox' : 'rounded border-gray-300 text-[var(--color-primary-searchmind)] focus:ring-[var(--color-primary-searchmind)]'}
                        checked={included}
            ...
            <td className={isCobalt ? 'apex-markets-table__cell-market' : 'px-3 py-2 whitespace-nowrap font-medium'}>
                {marketRowLabel(row)}
            </td>
            {metricColumns.filter((m) => visibleMetrics[m.key]).map((m) => getCellFor(m.key))}
        </tr>
    );
}
