...
export async function GET(_request, { params }) {
    ...
    if (isDemoCustomerId(customerId)) {
        return Response.json(demoPriceShaperData());
    }
    ...
    try {
        ...
        console.error("price-shaper GET:", error);
        return Response.json(
            { error: error.message || "Failed to load Price Shaper data" },
            { status: 500 }
        );
    }
}
...
export async function PATCH(request, { params }) {
    ...
    try {
        ...
        console.error("price-shaper PATCH:", error);
        return Response.json(
